...
export default function GrowerOrdersPage() {
  ...
  const fetchOrders = async () => {
    try {
      const { data } = await orderApi.getGrower();
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12">
      ...
      <p className="mt-1 text-gray-600 text-sm">Manage orders from consumers looking for your fresh produce.</p>

      {orders.length === 0 ? (
        <div className="card mt-8 flex flex-col items-center justify-center py-16 text-center">
          <span className="text-5xl mb-4">🌾</span>
          <p className="text-base font-bold text-slate-850">No orders received yet</p>
          <p className="mt-1 text-sm text-slate-500 max-w-xs mx-auto">When consumers search and order your listings, they will show up here.</p>
        </div>
      ) : (
          ...
          {orders.map((order) => (
            <div key={order._id} className="card border border-slate-100 shadow-sm overflow-hidden p-0">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-slate-50/50 px-5 py-4 border-b border-slate-100 gap-2">
                <div>
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Ordered by</p>
                  <h3 className="text-base font-bold text-slate-800 mt-0.5">{order.consumerName}</h3>
                  <p className="text-xs text-slate-400 mt-0.5">{order.consumerId.email}</p>
                </div>
                <div className="flex flex-col sm:items-end gap-1.5">
                  ...
                  <p className="text-[10px] text-slate-400">
                    Received on {new Date(order.createdAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                  </p>
                </div>
              </div>
                  ...
                  {order.status === 'pending' && (
                    <>
                      <button
                        ...
                        className="btn-primary py-2 text-xs font-bold bg-primary-600 border-primary-600 hover:bg-primary-700"
                      >
                        Accept Order
                      </button>
                      <button
                        ...
                        className="btn-secondary py-2 text-xs font-bold text-rose-600 border border-slate-200 hover:bg-rose-50/20"
                      >
                        Cancel
                      </button>
                    </>
                  ...
                  {order.status === 'accepted' && (
                    <>
                      <button
                        ...
                        className="btn-primary py-2 text-xs font-bold bg-emerald-600 border-emerald-600 hover:bg-emerald-700"
                      >
                        Complete Order
                      </button>
                      <button
